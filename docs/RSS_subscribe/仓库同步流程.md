...
1. **实时进度反馈**：通过 SSE 推送 `fetching → fetched → analyzing → done`
2. **链式调度**：手动同步完成后自动调度下一次，形成循环
...
#### POST /repositories/sync (第 48-198 行)
...
async def sync_repositories(request: Request, user=Depends(verify_auth)):
    ...
    async def sync_task():
        # 1. 发送 fetching 进度
        await queue.put({"event": "progress", "data": {"phase": "fetching"}})

        # 2. 获取所有星标仓库（分页）
        all_repos = await _fetch_all_starred_repos(github_token)
        await queue.put({"event": "progress", "data": {"phase": "fetched", "total": len(all_repos)}})
        ...
        if result["new_count"] > 0:
            await queue.put({"event": "progress", "data": {"phase": "analyzing"}})
            await ai_service.analyze_repositories_batch(new_repos, concurrency=5)
...
| Celery 任务 | `backend/app/celery_app/repository_tasks.py` | 异步任务 |
