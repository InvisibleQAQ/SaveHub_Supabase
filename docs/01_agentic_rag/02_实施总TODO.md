...
- [x] T5-1 移除前端对旧 `/api/rag-chat/stream` 的调用
- [x] T5-2 标注/下线后端 `rag_chat` 路由（按发布策略选择软下线或硬下线）
- [x] T5-3 清理无用状态文案与前端类型
...
> Phase 6 执行记录（2026-02-10）：
> - 已完成 `T6-1`：按 `07_测试与验收清单.md` 完整执行并回填
> - 结果：A-F、G2、G3 通过；`G1(frontend:lint)` 因当前环境缺少 `eslint` 且网络受限暂未通过
